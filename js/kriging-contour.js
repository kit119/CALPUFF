...
// Variogram models -> https://pro.arcgis.com/en/pro-app/latest/tool-reference/3d-analyst/how-kriging-works.htm

function kriging_variogram_exponential(h, nugget, range, sill, A) {
  return nugget + ((sill-nugget)/range) * (1.0 - Math.exp( -(1.0/A) * (h/range)));
}
function kriging_variogram_exponential_x(h, nugget, range, sill, A) {
  if (h > 0) return nugget + ((sill-nugget)/range) * (1.0 - Math.exp( -(1.0/A) * (h/range)));
  else return 0
}
function kriging_variogram_gaussian(h, nugget, range, sill, A) {
  return nugget + ((sill-nugget)/range) * (1.0 - Math.exp( -(1.0/A) * Math.pow(h/range, 1)));
}
function kriging_variogram_gaussian_x(h, nugget, range, sill, A) {
  if (h > 0) return nugget + ((sill-nugget)/range) * (1.0 - Math.exp( -(1.0/A) * Math.pow(h/range, 2)));
  else return 0
}
...
function kriging_variogram_spherical_x(h, nugget, range, sill, A) {
  if (h > range) return nugget + (sill-nugget)/range;
  else if(h <= range && h > 0) return nugget + ((sill-nugget)/range) * (1.5 * (h/range) - 0.5*Math.pow(h/range, 3));
  else return 0
}
function kriging_variogram_circular(h, nugget, range, sill, A) {
  if (h > range) return nugget + (sill-nugget)/range;
  return nugget + 2*((sill-nugget)/range)/Math.PI * ((h/range)*Math.sqrt(1-Math.pow(h/range,2)) + Math.asin(h/range));
}
function kriging_variogram_circular_x(h, nugget, range, sill, A) {
  if (h > range) return nugget + (sill-nugget)/range;
  else if(h <= range && h > 0) return nugget + 2*((sill-nugget)/range)/Math.PI * ((h/range)*Math.sqrt(1-Math.pow(h/range,2)) + Math.asin(h/range));
  else return 0
}
function kriging_variogram_linear(h, nugget, range, sill, A) {
  if (h > range) return nugget + (sill-nugget)/range;
  return nugget + ((sill-nugget)/range) * (h/range);
}
function kriging_variogram_linear_x(h, nugget, range, sill, A) {
  if (h > range) return nugget + (sill-nugget)/range;
  else if(h <= range && h > 0) return nugget + ((sill-nugget)/range) * (h/range);
  else return 0
}
...
kriging.train = function (t, x, y, model, sigma2, alpha) {
  ...
  case "gaussian":
      ...
      break;
  case "gaussian_x":
      variogram.model = kriging_variogram_gaussian_x;
      break;
  case "exponential":
      variogram.model = kriging_variogram_exponential;
      break;
  case "exponential_x":
      variogram.model = kriging_variogram_exponential_x;
      break;
  case "spherical":
      variogram.model = kriging_variogram_spherical;
      break;
  case "spherical_x":
      variogram.model = kriging_variogram_spherical_x;
      break;
  case "linear":
      variogram.model = kriging_variogram_linear;
      break;
  case "linear_x":
      variogram.model = kriging_variogram_linear_x;
      break;
  case "circular":
      variogram.model = kriging_variogram_circular;
      break;
  case "circular_x":
      variogram.model = kriging_variogram_circular_x;
      break;
  }
...
function _getKrigingGridInfo(featureCollection,weight,krigingParams){
  ...
  featureCollection.features.forEach(feature => {
      ...
      if(extent[3]<feature.geometry.coordinates[1])
        ...
        //console.log(extent)
      extent=extentKriging(extent,0.1) //extrapolate
        //console.log(extent)
...
function extentKriging(X,P){ //kit119
let D=P*Math.sqrt(Math.pow(X[2]-X[0],2)+Math.pow(X[3]-X[1],2))  
return extent=[X[0]-D,X[1]-D,X[2]+D,X[3]+D] 
}
